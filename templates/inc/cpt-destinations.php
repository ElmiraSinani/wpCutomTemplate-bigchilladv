...
function trip_meta_box_callback( $post ) {
	...
	$imgTitle       = get_post_meta( $post->ID, '_img_title_val', true );        
        ...
        $disVal  = get_post_meta( $post->ID, '_discount_val', true );
        $disColorVal  = get_post_meta( $post->ID, '_discount_color_val', true );
        
        $itineraryFromVal = get_post_meta( $post->ID, '_itinerary_from_val', true );
        $itineraryToVal   = get_post_meta( $post->ID, '_itinerary_to_val', true );
        ...
        ?>
            <style type="text/css">
                ...
                #discount_field, #discount_color_field{width: 150px;margin-right: 8px;}
                .wp-color-result{margin:0px !important;}
                .cptItem-data{padding-left: 18%;}
            </style>
        <?php
	...
	echo '<input type="number" id="waitlist_num_field" name="waitlist_num_field" value="' . esc_attr( $waitlistNumVal ) . '"  /></p>';
        ...
        echo '<p class="cptItem"><label for="discount_field">';
	_e( 'Trip Discount/Label Color', 'bca' );
	echo '</label> ';
	echo '<input type="text" id="discount_field" name="discount_field" value="' . esc_attr( $disVal ) . '"  />';
        echo '<input type="text" id="discount_color_field" name="discount_color_field" value="' . esc_attr( $disColorVal ) . '" class="discount-color-field"  /></p>';
	...
	echo '<select id="itinerary_field" name="itinerary_field" >';
        ...
        echo '<p class="cptItem-data">
                <input type="text" id="from" name="itinerary_from_field" placeholder="FROM" value="'.$itineraryFromVal.'" >
                <label for="to"> - </label>
                <input type="text" id="to" name="itinerary_to_field" placeholder="To" value="'.$itineraryToVal.'"></p>';
...
function trip_save_meta_box_data( $post_id ) {
	...
	$waitlist_num_data  = isset($_POST['waitlist_num_field']) ? htmlspecialchars( $_POST['waitlist_num_field'] ) : "";
        ...
        $discount_data    = isset($_POST['discount_field']) ?  $_POST['discount_field']  : "";
        $discount_color    = isset($_POST['discount_color_field']) ?  $_POST['discount_color_field']  : "";
        
        $itinerary_from   = isset($_POST['itinerary_from_field']) ?  $_POST['itinerary_from_field']  : "";
        $itinerary_to     = isset($_POST['itinerary_to_field']) ?  $_POST['itinerary_to_field']  : "";
	...
	update_post_meta( $post_id, '_activities_val', $activities );    
        ...
        update_post_meta( $post_id, '_discount_val', $discount_data );
	update_post_meta( $post_id, '_discount_color_val', $discount_color );       
        
	update_post_meta( $post_id, '_itinerary_from_val', $itinerary_from );         
	update_post_meta( $post_id, '_itinerary_to_val', $itinerary_to );         
